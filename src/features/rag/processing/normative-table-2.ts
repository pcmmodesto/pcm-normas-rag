  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.$executeRaw`
      ...
      values (
        ...
        ${"SERVICE_ENTRANCE_SIZING"},
        ${"2025.1"},
        ${"NT.00001.EQTL-09-Fornecimento-de-Energia-Eletrica-em-Baixa-Tensao"},
        ${"127/220V"},
        ${null},
        ${"CARGA_INSTALADA_KW"},
...
async function _runNormativeTableSchema() {
  ...
  await prisma.$executeRawUnsafe(`
    CREATE TABLE IF NOT EXISTS normative_tables (
      ...
      voltage TEXT,
      category TEXT,
      scope TEXT NOT NULL DEFAULT 'STATE_SPECIFIC',
      utility_group TEXT,
      applicable_ufs TEXT[] NOT NULL DEFAULT ARRAY[]::TEXT[],
      service_type TEXT,
      revision_label TEXT,
  ...
  for (const statement of [
    ...
    `CREATE INDEX IF NOT EXISTS normative_tables_scope_utility_group_idx ON normative_tables(scope, utility_group)`,
